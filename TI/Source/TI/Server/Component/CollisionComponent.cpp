...
void CollisionComponent::setRenderCollisionBoxEnabled(bool renderBoxEnabled)
{
	this->renderCollisionBox = renderBoxEnabled;
}

bool CollisionComponent::isRenderCollisiobBoxEnabled() const
{
	return renderCollisionBox;
}

void CollisionComponent::setRenderCollisions(bool renderCollisions)
{
	this->renderCollisions = renderCollisions;
}

bool CollisionComponent::isRenderCollisionsEnabled() const
{
...
void CollisionComponent::setCollisionBox(CollisionBox collisionBox)
{
	this->collisionBox = collisionBox;
}

CollisionBox CollisionComponent::getCollisionBox() const
{
	return collisionBox;
}

CollisionResult CollisionComponent::resolveCollision(const glm::vec3& position, const glm::vec3& velocity, float dt)
{
	...
	for (int x = -1 - static_cast<int>(collisionBox.getSize().x / 2.0f + xRangeMin); x <= 1 + static_cast<int>(collisionBox.getSize().x / 2.0f + xRangeMax); ++x)
	{
		for (int y = -1 - static_cast<int>(collisionBox.getSize().y / 2.0f + yRangeMin); y <= 1 + static_cast<int>(collisionBox.getSize().y / 2.0f + yRangeMax); ++y)
		{
			for (int z = -1 - static_cast<int>(collisionBox.getSize().z / 2.0f + zRangeMin); z <= 1 + static_cast<int>(collisionBox.getSize().z / 2.0f + zRangeMax); ++z)
			{
				...
				if (transformComponent->getCurrentBlockGrid()->getBlock(blockPosition) != 0)
				{
					if (renderCollisions)
					{
...
bool CollisionComponent::checkCollision(const glm::vec3& box1pos, const glm::vec3& box2pos, const CollisionBox& box1, const CollisionBox& box2)
{
...
glm::vec3 CollisionComponent::calculateAabbDistanceTo(const glm::vec3& box1pos, const glm::vec3& box2pos, const CollisionBox& box1, const CollisionBox& box2)
{
	glm::vec3 distance;
...
std::optional<RayCollisionResult> CollisionComponent::checkRayVsAabb(const glm::vec3& origin, const glm::vec3& direction, const glm::vec3& box2pos, const CollisionBox& box2)
{
	glm::vec3 near;
